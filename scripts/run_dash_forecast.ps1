...
$OutputDir = if ($env:LGF_OUTPUT_DIR) { $env:LGF_OUTPUT_DIR } else { "outputs" }
$HostName = if ($env:LGF_DASH_HOST) { $env:LGF_DASH_HOST } else { "127.0.0.1" }
$Port = if ($env:LGF_FORECAST_DASH_PORT) { $env:LGF_FORECAST_DASH_PORT } else { "8053" }
...
& $Python app\dash_forecast.py --data-path $DataPath --output-dir $OutputDir --host $HostName --port $Port
